...
function init() {
	document.querySelectorAll('.mod-interactive-map-container').forEach((obj, key) => {
		...
		onImageChildrenLoaded(obj)
			.then(() => {
				...
				obj.querySelector('.mod-interactive-map-reset-button').addEventListener('click', (event) => {
					panzoom.reset(true);
