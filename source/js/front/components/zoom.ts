...
function onImageChildrenLoaded(parent) {
	return new Promise(async (resolve) => {
		...
		if (imageNodes.length === 0)
			throw new Error("No child image elements found.");
...
function init() {
	document
		.querySelectorAll(".mod-interactive-map-container")
		.forEach((obj, key) => {
			const zoomObj = obj.querySelector(".mod-interactive-map-zoomable");
			const overflowContainer = obj.querySelector(
				".mod-interactive-map-overflower",
			);
			const resetButton = obj.querySelector(
			...
			onImageChildrenLoaded(obj)
				.then(() => {
					const panzoom = Panzoom(zoomObj, {
						...
						minScale: 1,
						maxScale:
							obj.querySelector("img").naturalWidth /
							obj.querySelector("img").clientWidth,
						increment: 0.1,
						contain: "outside",
					});

					obj
						.querySelector(".mod-interactive-map-reset-button")
						.addEventListener("click", (event) => {
							panzoom.reset(true);
							handleClick(false, resetButton, overflowContainer);
						});

					obj.querySelector(".zoom-in").addEventListener("click", (event) => {
						panzoom.zoomIn();
						handleClick(true, resetButton, overflowContainer);
					});

					obj.querySelector(".zoom-out").addEventListener("click", (event) => {
						panzoom.zoomOut();
	...
	document
		...
		.forEach((obj, key) => {
			if (
				...
				obj
					.querySelector(".mod-iteractive-map-buttons")
